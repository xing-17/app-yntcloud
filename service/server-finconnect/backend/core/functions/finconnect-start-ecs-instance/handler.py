...
def main(event, context):
    ...
    # ------------------------------------------------------
    try:
        response = ecs.start_instance_with_options(
    ...
    # Prepares instance
    # ------------------------------------------------------
    # Running command to start FinConnect Server
    try:
        response = ecs.run_command_with_options(
            tmp_req=EcsModels.RunCommandRequest(
                instance_id=[instance_id],
                region_id=region_id,
                command_content="cd /root/workspace/app-finconnect && make start",
                type="RunShellScript",
                timeout=300,
            ),
            runtime=UtilModels.RuntimeOptions(autoretry=True),
        ...
        logger.info(f"Command invoked on '{instance_id}' OK, invoke_id={invoke_id}")
    except Exception as e:
        raise RuntimeError(f"Failed to run command on '{instance_id}': {e}") from e
